...
- Use reserved placeholders such as `example.com` and provider-specific placeholder keys.
- Distinguish Flutter build-time config from Vercel serverless secrets and Convex backend env.
- Keep `CONVEX_URL`, `FIREBASE_*`, and `REPLAYGLOWZ_APP_URL` documented as build-time values.
- Keep `GOOGLE_CLIENT_ID` and `GOOGLE_CLIENT_SECRET` documented as serverless OAuth values.
