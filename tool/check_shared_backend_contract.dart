...
void main() {
  ...
  if (!backendRoot.existsSync()) {
    stderr.writeln(
      ...
      'Set TUBEFLOW_BACKEND_ROOT to the Convex backend directory if needed.',
    );
    exitCode = 1;
    return;
  }
...
Directory _resolveBackendRoot() {
  final fromEnv = Platform.environment['TUBEFLOW_BACKEND_ROOT'];
  if (fromEnv != null && fromEnv.trim().isNotEmpty) {
    return Directory(fromEnv.trim());
  }

  final cwd = Directory.current.uri;
  return Directory.fromUri(cwd.resolve('../tubeflow/packages/backend/convex'));
}
