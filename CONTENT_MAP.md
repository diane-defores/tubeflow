...
| Product context | `PRODUCT.md` | Reviewed product scope, target user, workflows, non-goals, and claim boundaries | Markdown artifact | Evidenced product behavior and specs | Product scope, workflows, or claim boundaries change |
| Business context | `BUSINESS.md` | Business assumptions, monetization unknowns, target audience, and commercial claim limits | Markdown artifact | Diane decisions plus product evidence | Diane confirms model, segment, ICP, pricing, or commercial strategy |
| Brand context | `BRANDING.md` | Voice, messaging pillars, tagline candidates, and brand claim boundaries | Markdown artifact | Diane decisions plus product and business context | Diane confirms tone, tagline, language priorities, or positioning |
| GTM context | `GTM.md` | Reviewed product-led GTM framing, activation path, proof points, objections, and KPI candidates | Markdown artifact | Product evidence and repo-visible routes/config | Offer, activation, channels, or product proof points change |
...
| Work trackers | `TASKS.md`, `AUDIT_LOG.md` | Active backlog and audit trace | Markdown | Current work management, not canonical product truth | Work status changes; do not treat as decision contracts |

## Semantic Architecture

| Cluster | Pillar page | Supporting pages | Target intent | Internal link rule | Status |
|---|---|---|---|---|---|
| Product truth and setup | `README.md` | `PRODUCT.md`, `CLAUDE.md`, `.env.example`, `vercel.json` | Operator and contributor understanding | README points to deeper setup or architecture context when needed | live |
| Product and market context | `PRODUCT.md` | `BUSINESS.md`, `BRANDING.md`, `GTM.md` | Keep claims aligned across product, business, brand, and GTM | Downstream copy should inherit product claim boundaries before expanding | live |
| Active feature decision-making | `specs/flutter-web-youtube-auth-redirect-spec.md` | `specs/feedback-v1.md`, future feature specs | Implementation and review intent | Specs should reference product context and related routes/workflows | live |
| Product feedback loop | `/feedback` | `/feedback/admin`, `specs/feedback-v1.md`, `PRODUCT.md` | Support and product-learning intent | User-facing feedback entry should stay consistent with admin review workflow and specs | live |
| Release communication | `CHANGELOG.md` | `README.md`, `PRODUCT.md` when behavior materially changes | Change-awareness intent | User-visible changes belong in changelog; structural changes may also require README updates | live |
...
- Use `PRODUCT.md` for product framing before writing feature docs, support copy, release copy, or GTM copy.
- Use `BUSINESS.md` only for commercial assumptions that are clearly marked as confirmed or open.
- Use `BRANDING.md` for tone and safe phrasing, but do not treat tagline candidates as approved until reviewed.
- Use `GTM.md` for positioning language and activation framing, while keeping revenue and channel claims narrow.
...
- Do not treat `TASKS.md` or `AUDIT_LOG.md` as canonical content sources for product claims.

## Cross-Surface Update Rules

| Trigger | Check these surfaces |
|---|---|
| New user-visible feature | `README.md`, `CHANGELOG.md`, related `specs/*.md`, `PRODUCT.md` |
| Auth or YouTube-connect change | `README.md`, `CLAUDE.md`, relevant spec, `GTM.md` if activation messaging changes |
| Feedback flow change | `/feedback` and `/feedback/admin` behavior, `specs/feedback-v1.md`, `PRODUCT.md`, `CHANGELOG.md` if user-visible |
| Deployment or env change | `README.md`, `.env.example`, `CLAUDE.md` |
| Repositioning or target-user change | `PRODUCT.md`, `BUSINESS.md`, `BRANDING.md`, `GTM.md`, then any user-facing README or changelog framing |
| Monetization or pricing decision | `BUSINESS.md`, `GTM.md`, `BRANDING.md`, then product or README surfaces only if user-visible |
| Voice, tagline, or language-priority decision | `BRANDING.md`, then user-facing app copy, README, and GTM surfaces if affected |

## Current Gaps

- `BUSINESS.md` remains draft until Diane confirms monetization, primary segment, and individual-versus-team trajectory.
- `BRANDING.md` remains draft until Diane confirms tone, tagline, and language priority.
...
- KPI candidates in `GTM.md` are recommended validation metrics, not confirmed analytics instrumentation.
